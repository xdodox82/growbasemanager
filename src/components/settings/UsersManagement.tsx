  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      try {
        ...
        else console.warn('[DELETE USER] RPC failed, using fallback:', error || data);
      } catch (rpcErr) {
        console.warn('[DELETE USER] RPC exception, using fallback:', rpcErr);
      }

      // Fallback: zmaž z user_roles (odoberie prístup do GrowBase)
      if (!deleted) {
      ...
      toast({ title: deleted ? 'Používateľ zmazaný' : 'Prístup odobratý', description: deleted ? 'Používateľ bol odstránený.' : 'Prístup do GrowBase bol odobratý.' });
    } catch (err: any) {
