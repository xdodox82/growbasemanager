/*
  # Fix admin_create_user_v3 to use correct pgcrypto schema (extensions)

  1. Purpose
    - Update admin_create_user_v3 to use extensions.crypt() and extensions.gen_salt()
    - pgcrypto is installed in extensions schema, not public schema
    - Fix "function gen_salt(unknown) does not exist" error

  2. Changes
    - Change public.crypt() to extensions.crypt()
    - Change public.gen_salt() to extensions.gen_salt()
    - Add extensions to search_path for convenience

  3. Verification
    - Tested that extensions.gen_salt('bf') works correctly
    - pgcrypto version 1.3 confirmed in extensions schema
*/

-- Drop existing function
DROP FUNCTION IF EXISTS admin_create_user_v3(text, text, text, text);

-- Create function with correct pgcrypto schema paths
...
BEGIN
  ...
  BEGIN
    -- Use extensions.crypt() and extensions.gen_salt() - correct schema for Supabase
...
COMMENT ON FUNCTION admin_create_user_v3 IS 'Creates a new user with pgcrypto password encryption from extensions schema. Admin only. Includes detailed debugging and error messages.';
