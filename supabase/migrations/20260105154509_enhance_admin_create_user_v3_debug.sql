/*
  # Enhanced admin_create_user_v3 with Debug and Error Handling

  1. Purpose
    - Add detailed RAISE NOTICE statements for debugging
    - Improve error messages with specific details
    - Add ON CONFLICT handling for auth.users
    - Validate email format and password strength
    - Simplify metadata to avoid conflicts

  2. Changes
    - Add RAISE NOTICE at each step
    - Add email format validation
    - Add password length validation (min 6 chars)
    - Add ON CONFLICT (id) DO NOTHING to auth.users insert
    - Simplify raw_app_meta_data and raw_user_meta_data
    - Return detailed error messages with error_code

  3. Benefits
    - Easy debugging - see exactly where function fails
    - Clear error messages for users
    - Prevent duplicate key errors with ON CONFLICT
...
-- Create enhanced function with better debugging
...
COMMENT ON FUNCTION admin_create_user_v3 IS 'Creates a new user with detailed debugging, error handling, and cascade cleanup. Admin only. Returns detailed error messages with error_code and step information for easy troubleshooting.';
