/*
  # Add generate_planting_plan RPC function
  
  1. Overview
     Creates an RPC function to generate planting plans based on orders with delivery dates
     within a specified date range.
  
  2. Function Details
     - Name: generate_planting_plan
     - Parameters:
       - p_start_date: Start date for delivery date range
       - p_end_date: End date for delivery date range
     - Returns: Table with planting plan details
  
  3. Logic
     - Finds all orders with delivery_date between start and end dates
     - For each order item with a crop:
       - Calculates sow_date = delivery_date - days_to_harvest
       - Estimates tray count based on quantity and expected_yield
       - Calculates seed amounts based on tray_count and seed_density
       - Creates or updates planting_plan record
     - Returns summary of created/updated plans
